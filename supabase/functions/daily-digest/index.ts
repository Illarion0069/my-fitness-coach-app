...
serve(async (req) => {
  ...
  try {
    ...
    if (visitors.size === 0) {
      ...
      verdict.push(
        `Люди ${bookingOpen} раз нажимали кнопку записи, но до конца бронирование не дошёл никто — они бросали форму на шаге выбора даты/времени или заполнения данных. Стоит упростить форму записи.`,
      );
